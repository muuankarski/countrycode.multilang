...
countrycode <- function (sourcevar, origin, destination, warn=FALSE){
    ...
    origin_codes <- names(countrycode.multilang::countrycode_data)[!(names(countrycode.multilang::countrycode_data) %in% c("continent","region","regex"))] 
    destination_codes <- names(countrycode.multilang::countrycode_data)[!(names(countrycode.multilang::countrycode_data) %in% c("regex"))]
    ...
    if (origin == 'country.name'){
        dict = na.omit(countrycode.multilang::countrycode_data[,c('regex', destination)])
    }else{
        dict = na.omit(countrycode.multilang::countrycode_data[,c(origin, destination)])
    }
